 ...
 */
export type Gate = CustomGate | EnvGate | ExperimentGate | PercentageGate | UserGate | VersionGate;
...
export interface RemoteFlagPayload {
  ...
  flags: Record<
    string,
    {
       ...
       */
      gates?: Array<
        | { allow: Env[]; priority?: number; type: 'env' }
        | {
            allowBuckets: string[];
            experimentKey: string;
            priority?: number;
        ...
        | { max?: string; min?: string; priority?: number; type: 'version' }
        | { priority?: number; type: 'percentage'; value: number }
      >;
    }
  >;
